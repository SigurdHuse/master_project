...
try_different_learning_rates(config=config, dataloader=dataloader, PDE=black_scholes_1D,
                             filename1="important_results/test_MSE.txt", filename2="important_results/test_epochs.txt",
                             learning_rates=[1e-3], batch_sizes=[256],
                             validation_data=validation_data, test_data=test_data, epochs = 150_000) """
...
try_different_learning_rates(config=config, dataloader=dataloader, PDE=black_scholes_1D,
                             ...
                             learning_rates=[5e-3, 1e-3, 5e-4], batch_sizes=[128, 256, 512],
...
try_different_learning_rates(config=config, dataloader=dataloader, PDE=black_scholes_1D,
                             ...
                             learning_rates=[2e-3, 1e-3, 9e-4], batch_sizes=[256, 512, 1024],
...
try_multiple_activation_functions(config=config, dataloader=dataloader, PDE=black_scholes_1D,
                                  ...
                                  layers=[1, 2, 4, 8], validation_data=validation_data, test_data=test_data, epochs = 200_000) """
...
computing_the_greeks(config=config, dataloader=dataloader, PDE=black_scholes_1D,
                     filename="important_results/greeks_1.txt", validation_data=validation_data, test_data=test_data, epochs=400_000)
